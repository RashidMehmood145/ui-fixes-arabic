<?php
include('header.php');
?>
		<main role="main">
		<div class="container">	
			<section class="media-hero-section">
				<div class="media-hero-gray-box"></div>
				<div class="media-hero-line-box"></div>
				<div class="media-hero-content">
					<h2 class="media-hero-heading">Media</h2>
					<h3 class="media-hero-sub-heading">Media (News)</h3>
					<div class="media-hero-short-details">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim</div>
				</div>
				
				<div class="media-hero-image">
					<img src="./assets/images/media-page-main-banner.svg">
					<div class="bar-box-bottom-right"></div>
				</div>
			</section>
		</div>
			
			<!---------Content here------------>
			<section class="after-media-hero-section">
				<div class="container">
					<div class="row vertical-tabs">
						<div  class="col-md-9 news-category-col">
							<div class="tab-content">
								<div class="tab-pane active" id="category1" role="tabpanel">
									<div class="row">
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div> 
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 1</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
									</div>
									<div class="row">
										<div class="col-md-12 mb-4">
											<div class="load-more-media-button-wrapper">
												<a class="load-more-media-cta">Load more Media</a>
											</div>
										</div>
									</div>
									<!--------------Row End-------------->
								</div>
								<div class="tab-pane" id="category2" role="tabpanel">
									<div class="row">
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div> 
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 2</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
									</div>
									<div class="row">
										<div class="col-md-12 mb-4">
											<div class="load-more-media-button-wrapper">
												<a class="load-more-media-cta">Load more Media</a>
											</div>
										</div>
									</div>
									<!--------------Row End-------------->
								</div>
								<div class="tab-pane" id="category3" role="tabpanel">
									<div class="row">
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div> 
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 3</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
									</div>
									<div class="row">
										<div class="col-md-12 mb-4">
											<div class="load-more-media-button-wrapper">
												<a class="load-more-media-cta">Load more Media</a>
											</div>
										</div>
									</div>
									<!--------------Row End-------------->
								</div>
								<div class="tab-pane" id="category4" role="tabpanel">
									<div class="row">
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div> 
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 4</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
									</div>
									<div class="row">
										<div class="col-md-12 mb-4">
											<div class="load-more-media-button-wrapper">
												<a class="load-more-media-cta">Load more Media</a>
											</div>
										</div>
									</div>
									<!--------------Row End-------------->
								</div>
								<div class="tab-pane" id="category5" role="tabpanel">
									<div class="row">
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div> 
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<!---------------------------->
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-1.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-2.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
										<div class="col-md-4 mb-4">
											<div class="news-category-box-wrapper">
												<div class="news-category-box-image">
													<div class="news-category-label">Category 5</div>
													<img src="./assets/images/category-3.svg">
												</div>
												<div class="news-category-box-content">
													<div class="news-date">10/09/2022</div>
													<div class="news-title">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													<div class="news-content">Lorem ipsum dolor sit amet, consectetur adipiscing</div>
													
												</div>
											</div>
										</div>
									</div>
									<div class="row">
										<div class="col-md-12 mb-4">
											<div class="load-more-media-button-wrapper">
												<a class="load-more-media-cta">Load more Media</a>
											</div>
										</div>
									</div>
									<!--------------Row End-------------->
								</div>
							</div>		
						</div>
						<div class="col-md-3">
							<!--------------------------->
							<ul class="nav nav-tabs" role="tablist">
								<li class="nav-item">
								  <a class="nav-link active" data-toggle="tab" href="#category1" role="tab" aria-controls="Category 1">Category 1</a>
								</li>
								<li class="nav-item">
								  <a class="nav-link" data-toggle="tab" href="#category2" role="tab" aria-controls="Category 2">Category 2</a>
								</li>
								<li class="nav-item">
								  <a class="nav-link" data-toggle="tab" href="#category3" role="tab" aria-controls="Category 3">Category 3</a>
								</li>
								<li class="nav-item">
								  <a class="nav-link" data-toggle="tab" href="#category4" role="tab" aria-controls="Category 4">Category 4</a>
								</li>

								<li class="nav-item">
								  <a class="nav-link" data-toggle="tab" href="#category5" role="tab" aria-controls="Category 5">Category 5</a>
								</li>								
							</ul>
						</div>
					</div>
					<!--------------------------->
				</div>
			</section>
			
		</main>
			<?php
include('footer.php');
?>